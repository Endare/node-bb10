 ...
 * @param {Object} callback The callback function that will be invoked when the PPG responds.
 */
PushInititator.prototype.push = function(message, callback) {
	var deliverBefore = new Date();
	deliverBefore.setTime(deliverBefore.getTime()+24*3600);
	
	var body = message.toPAPMessage({
		'push-id': message.id,
		'source-reference': this.applicationId,
		'deliver-before-timestamp': deliverBefore.format('Y-m-dTH:i:sZ')
	});
